...
use Riclep\Storyblok\Traits\xRequestsStories;

abstract class xBlock implements \JsonSerializable, \Iterator, \ArrayAccess, \Countable
{
	use xProcessesBlocks;
	use xRequestsStories;
	...
	public function makeComponentPath($componentPath)
	{
		...
		$this->content->each(function($block) use ($componentPath) {
			if ($block instanceof xBlock || $block instanceof xAsset) {
	...
	public function page() {
		if ($this->parent instanceof xPage) {
			return $this->parent;
		}
